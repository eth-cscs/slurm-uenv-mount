...
pkg_name="slurm-uenv-mount-${version}"
mkdir -p "${build_path}"
...
spec_file="${build_path}/SPECS/slurm-uenv-mount.spec"
cp slurm-uenv-mount.spec.in "${spec_file}"
sed -i "s|UENVMNT_VERSION|${version}|g" "${spec_file}"
