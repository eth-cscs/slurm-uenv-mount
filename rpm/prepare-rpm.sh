...
usage="$(basename "$0") [-h] [-s,--skip-binary] [--slurm-version] dstdir-- Helper script to create a source and binary rpm of the project.
...
(
  ...
  tar --transform="s#.#slurm-uenv-mount-${SLURM_UENV_MOUNT_VERSION}#" --exclude=".git/" \
    -czf SOURCES/slurm-uenv-mount-"${SLURM_UENV_MOUNT_VERSION}".tar.gz \
    -C "${_projectdir}" .
